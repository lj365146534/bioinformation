...
#	system ("tail -n 30000 $indir/$eachfile > $outdir/$basename/cat_file.fa");
	...
	system ("head -3000 $outdir/$basename/trim_1.fastq > $outdir/$basename/cat_file.fq");
...
#	system ("velvetg $outdir/$basename/velvet  ");
	...
	system ("blastn -query $outdir/$basename/velvet/454AllContigs.fna -out $outdir/$basename/velvet/contigs.blast -db $outdir/../B_flu/B_flu.fa -outfmt '6 stitle sseqid' -max_target_seqs 1 -num_threads $threads");
...
###
	...
	system ("velvetg $outdir/$basename/velvet -max_branch_length 250 -max_divergence 0.4 -max_gap_count 10 -cov_cutoff 55   -scaffolding yes ");
#	system ("velvetg $outdir/$basename/velvet  ");
	system ("blastn -query $outdir/$basename/velvet/contigs.fa  -db $outdir/../B_flu/B_flu.fa -outfmt '6 stitle sseqid' -max_target_seqs 1 -num_threads $threads >> $outdir/$basename/velvet/contigs.blast");
###


	my $aa = "$outdir/$basename/velvet";
	grep_flu_fregment_2::grep_flu ( $aa, "contigs.blast");
	mkdir  "$outdir/$basename/first_map";
	system ("blastdbcmd -db $outdir/../B_flu/B_flu.fa -entry_batch $aa/blast_list > $outdir/$basename/blast.fa");
...
=edit	
	...
	close OUT;
	#
	system ("blastn -query $outdir/$basename/second_map/unmap.fa -out $outdir/$basename/unmap.blast -db $outdir/../B_flu/B_flu.fa -outfmt '6 stitle sseqid' -max_target_seqs 1 -num_threads $threads");
